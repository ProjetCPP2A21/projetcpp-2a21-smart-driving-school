...
QSqlQueryModel* Apprenant::rechercher(const QString &prefixeId)
{
    QSqlQueryModel* model = new QSqlQueryModel();
    QSqlQuery query;
    query.prepare("SELECT ID_APPRENANT, NOM, PRENOM, TO_CHAR(DATE_NAISSANCE,'DD/MM/YYYY') AS DATE_NAISSANCE, TELEPHONE, SEXE, ADRESSE "
                  "FROM APPRENANTS WHERE TO_CHAR(ID_APPRENANT) LIKE :prefixeId || '%'");
    query.bindValue(":prefixeId", prefixeId);
    query.exec();
