...
// Petit helper pour éviter de répéter le nom de la connexion
...
{
    QSqlQuery query(db());
    query.prepare("UPDATE APPRENANTS "
                  "SET NOM=:nom, PRENOM=:prenom, DATE_NAISSANCE=TO_DATE(:dateNaissance,'DD/MM/YYYY'), "
                  "    TELEPHONE=:telephone, SEXE=:sexe, ADRESSE=:adresse "
...
bool Apprenant::modifierAvecID(int nouvelId)
{
    QSqlQuery query(db());
    query.prepare("UPDATE APPRENANTS "
                  "SET ID_APPRENANT=:nouvelId, NOM=:nom, PRENOM=:prenom, DATE_NAISSANCE=TO_DATE(:dateNaissance,'DD/MM/YYYY'), "
                  "    TELEPHONE=:telephone, SEXE=:sexe, ADRESSE=:adresse "
                  "WHERE ID_APPRENANT=:id");
    query.bindValue(":id", id);
...
QSqlQueryModel* Apprenant::afficher()
{
    ...
    model->setQuery(
        "SELECT ID_APPRENANT, NOM, PRENOM, "
...
QSqlQueryModel* Apprenant::rechercher(int id)
{
    QSqlQueryModel* model = new QSqlQueryModel();
    QSqlQuery query(db());
    query.prepare("SELECT ID_APPRENANT, NOM, PRENOM, "
                  "TO_CHAR(DATE_NAISSANCE,'DD/MM/YYYY') AS DATE_NAISSANCE, "
                  "TELEPHONE, SEXE, ADRESSE "
...
QSqlQueryModel* Apprenant::rechercherParPrefixe(const QString &prefix)
{
    ...
    if (!query.exec()) {
        qDebug() << "Erreur recherche par préfixe:" << query.lastError().text();
    }
...
QMap<QString,int> Apprenant::statistiquesSexe()
{
    ...
    if (query.exec()) {
        ...
        qDebug() << "Erreur statistiquesSexe:" << query.lastError().text();
    }
    return stats;
}
