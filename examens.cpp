...
void examens::setupArduino()
{
    ...
    arduino->setPortName("COM5"); // COM détecté
...
void examens::lireArduino()
{
    ...
    while (arduino->canReadLine()) {
        ...
        if (uid.startsWith("UID:")) {
            uid = uid.mid(4).toUpper(); // supprimer "UID:"
...
{
    QStringList idsAutorises = {"32F3F404"};

    if (idsAutorises.contains(id)) {
        qDebug() << "ID valide !";

        // Envoyer commande ouverture + fermeture à Arduino
        arduino->write("GO\n");

        // Premier message : porte ouverte
        QMessageBox::information(this, "Porte", "Porte ouverte !");

        // Timer pour afficher message porte fermée après 5 secondes
