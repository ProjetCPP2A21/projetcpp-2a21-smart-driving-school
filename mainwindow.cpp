...
void MainWindow::setTest(Test *t)
{
    m_test = t;
}

int MainWindow::verifierApprenantId(const QString &id)
{
    QSqlQuery query(QSqlDatabase::database("oracle_connection"));
    query.prepare("SELECT COUNT(*) FROM APPRENANTS WHERE ID_EMPREINTE = :id");
    bool ok = false;
    int idEmp = id.toInt(&ok);
    if (!ok) return 0;
...
void MainWindow::on_testp_clicked()
{
    QString id = ui->ID_2->text().trimmed();
    if (id.isEmpty()) {
        QMessageBox::warning(this, "ID manquant", "Veuillez saisir l'ID emprunté dans le champ ID-2.");
        return;
    }

    int existe = verifierApprenantId(id);
    if (m_test) {
        m_test->envoyerAuArduino(QByteArray(existe ? "1" : "0"));
    } else {
        qDebug() << "Avertissement: aucun objet Test n'est associé à MainWindow.";
    }
}
