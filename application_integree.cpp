    ...
    , modelVehicules(nullptr)
{
    ...
    // Initialisation de la page Paiement
    QTableView* tableViewPaiement = nullptr;
    // Chercher dans la page paiement si elle existe
    if (ui->stackedWidget->count() > 6) {
        QWidget* pagePaiement = ui->stackedWidget->widget(6);
        if (pagePaiement) {
            tableViewPaiement = pagePaiement->findChild<QTableView*>("tabView");
            if (!tableViewPaiement) {
                tableViewPaiement = pagePaiement->findChild<QTableView*>("tableView_paiements");
            }
        }
    }
    if (tableViewPaiement) {
        tableViewPaiement->setSelectionBehavior(QAbstractItemView::SelectRows);
        tableViewPaiement->setSelectionMode(QAbstractItemView::SingleSelection);
        tableViewPaiement->setEditTriggers(QAbstractItemView::NoEditTriggers);
        // Connexion manuelle si le widget s'appelle tabView (nom original)
        if (tableViewPaiement->objectName() == "tabView") {
            connect(tableViewPaiement, &QTableView::clicked, this, &application_integree::on_tableView_paiements_clicked);
        }
        afficherPaiements();
    }
    // Initialiser les champs de date et heure si les widgets existent
    if (ui->stackedWidget->count() > 6) {
        QWidget* pagePaiement = ui->stackedWidget->widget(6);
        if (pagePaiement) {
            QDateEdit* dateWidget = pagePaiement->findChild<QDateEdit*>("Date");
            if (!dateWidget) dateWidget = pagePaiement->findChild<QDateEdit*>("dateEdit_date_paiement");
            if (dateWidget) dateWidget->setDate(QDate::currentDate());
            
            QTimeEdit* heureWidget = pagePaiement->findChild<QTimeEdit*>("heure");
            if (!heureWidget) heureWidget = pagePaiement->findChild<QTimeEdit*>("timeEdit_heure_paiement");
            if (heureWidget) heureWidget->setTime(QTime::currentTime());
        }
    }
}
...
// ==================== MENU PRINCIPAL ====================
// Navigation depuis le menu principal vers les différentes pages
...
// Bouton "Véhicules" dans le menu → Ouvre la page Véhicules (index 4)
...
// Bouton "Paiements" dans le menu → Ouvre la page Paiements (index 6)
void application_integree::on_pushButton_paiements_clicked()
{
    // Vérifier que la page existe
    if (ui->stackedWidget->count() <= 6) {
        QMessageBox::warning(this, "Attention", "La page Paiements n'existe pas encore dans l'interface.\n"
                              "Veuillez l'ajouter dans Qt Designer (Index 6).");
        return;
    }
    ...
    // Utiliser la fonction utilitaire pour trouver le tableau
    QTableView* tableView = findTableViewPaiement();
    if (tableView) {
...
void application_integree::afficherMoniteurs()
{
    QSqlQuery query("SELECT ID, NOM, PRENOM, SPECIALITE, TELEPHONE FROM MONITEUR");
    ui->table->setRowCount(0); // vider la table
    ...
    connect(ui->table, &QTableWidget::cellClicked, this, [this](int row, int){
        selectedRowMoniteur = row;
        ui->ID->setText(ui->table->item(row,0)->text());
        ui->nom->setText(ui->table->item(row,1)->text());
        ui->prenom->setText(ui->table->item(row,2)->text());
        ui->spec->setCurrentText(ui->table->item(row,3)->text());
        ui->telephone->setText(ui->table->item(row,4)->text());
    });
}

void application_integree::clearFieldsMoniteur()
{
    ui->ID->clear();
    ui->nom->clear();
    ui->prenom->clear();
    ui->spec->setCurrentIndex(-1); // vide le QComboBox
    ui->telephone->clear();
    selectedRowMoniteur = -1;
}

// --- Ajouter ---
void application_integree::on_ajouter_clicked()
{
    ...
    if (id <= 0 || nom.isEmpty() || prenom.isEmpty() || specialite.isEmpty() || tel.isEmpty()) {
        QMessageBox::warning(this,"Erreur","Veuillez remplir tous les champs !");
        return;
    }

    QSqlQuery query;
    query.prepare("INSERT INTO MONITEUR (ID, NOM, PRENOM, SPECIALITE, TELEPHONE) "
                  "VALUES (:id, :nom, :prenom, :spec, :tel)");
...
// --- Modifier ---
void application_integree::on_modifier_clicked()
{
    if (selectedRowMoniteur < 0) {
    ...
    QSqlQuery query;
    query.prepare("UPDATE MONITEUR SET NOM=:nom, PRENOM=:prenom, SPECIALITE=:spec, TELEPHONE=:tel WHERE ID=:id");
    query.bindValue(":nom", nom);
    query.bindValue(":prenom", prenom);
    query.bindValue(":spec", specialite);
    query.bindValue(":tel", tel);
...
// --- Supprimer ---
void application_integree::on_supprimer_clicked()
{
    if (selectedRowMoniteur < 0) {
        QMessageBox::warning(this,"Erreur","Veuillez sélectionner un moniteur !");
        return;
...
void application_integree::on_pushButton_retour_moniteurs_clicked()
{
    ui->stackedWidget->setCurrentIndex(1); // Retour au menu principal (index 1)
    clearFieldsMoniteur();                 // Nettoyer les champs
    ui->table->clearSelection();           // Nettoyer la sélection dans la table
}

// ==================== FONCTIONS UTILITAIRES PAIEMENT ====================
// Fonctions pour trouver les widgets de manière flexible et sécurisée

QTableView* application_integree::findTableViewPaiement() const
{
    // Chercher dans la page paiement
    if (ui->stackedWidget->count() > 6) {
        QWidget* pagePaiement = ui->stackedWidget->widget(6);
        if (pagePaiement) {
            // Essayer le nom original du projet paiement
            QTableView* tableView = pagePaiement->findChild<QTableView*>("tabView");
            if (tableView) {
                return tableView;
            }
            // Essayer le nom standard
            tableView = pagePaiement->findChild<QTableView*>("tableView_paiements");
            if (tableView) {
                return tableView;
            }
            // Chercher n'importe quel QTableView dans la page
            tableView = pagePaiement->findChild<QTableView*>();
            if (tableView) {
                return tableView;
            }
        }
...
QLineEdit* application_integree::findLineEditPaiement(const QString& name) const
{
    // Chercher dans la page paiement
    if (ui->stackedWidget->count() > 6) {
        QWidget* pagePaiement = ui->stackedWidget->widget(6);
        if (pagePaiement) {
            QLineEdit* widget = pagePaiement->findChild<QLineEdit*>(name);
...
QComboBox* application_integree::findComboBoxPaiement(const QString& name) const
{
    // Chercher dans la page paiement
    if (ui->stackedWidget->count() > 6) {
        QWidget* pagePaiement = ui->stackedWidget->widget(6);
        if (pagePaiement) {
            QComboBox* widget = pagePaiement->findChild<QComboBox*>(name);
            if (widget) return widget;
        }
    }
    return nullptr;
...
QDateEdit* application_integree::findDateEditPaiement(const QString& name) const
{
    // Chercher dans la page paiement
    if (ui->stackedWidget->count() > 6) {
        QWidget* pagePaiement = ui->stackedWidget->widget(6);
        if (pagePaiement) {
            QDateEdit* widget = pagePaiement->findChild<QDateEdit*>(name);
            if (widget) return widget;
        }
    }
    return nullptr;
...
QTimeEdit* application_integree::findTimeEditPaiement(const QString& name) const
{
    // Chercher dans la page paiement
    if (ui->stackedWidget->count() > 6) {
        QWidget* pagePaiement = ui->stackedWidget->widget(6);
        if (pagePaiement) {
            QTimeEdit* widget = pagePaiement->findChild<QTimeEdit*>(name);
            if (widget) return widget;
        }
    }
    return nullptr;
...
// ==================== PAGE PAIEMENT ====================
void application_integree::afficherPaiements()
{
    Paiement p;
...
void application_integree::on_pushButton_ajouter_paiement_clicked()
{
    // Récupérer les valeurs depuis l'UI avec fonctions utilitaires
    QLineEdit* idWidget = findLineEditPaiement("ID");
    if (!idWidget) {
        idWidget = findLineEditPaiement("lineEdit_id_paiement");
    }
    
    if (!idWidget) {
        QMessageBox::warning(this, "Erreur", "Widget ID introuvable !");
        return;
    }
    
    QString idText = idWidget->text().trimmed();
    bool ok;
    int id = idText.toInt(&ok);
    if (!ok || id <= 0) {
    ...
    QComboBox* typeWidget = findComboBoxPaiement("Type");
    if (!typeWidget) {
        typeWidget = findComboBoxPaiement("comboBox_type_paiement");
    }
    
    if (!typeWidget) {
        QMessageBox::warning(this, "Erreur", "Widget Type introuvable !");
        return;
    }
    
    QString type = typeWidget->currentText();
    if (type.isEmpty() || type == "modes de paiement") {
        QMessageBox::warning(this, "Erreur", "Veuillez sélectionner un type de paiement !");
        return;
    }

    QDateEdit* dateWidget = findDateEditPaiement("Date");
    if (!dateWidget) {
        dateWidget = findDateEditPaiement("dateEdit_date_paiement");
    }
    
    QString date;
    if (dateWidget) {
        date = dateWidget->date().toString("dd-MM-yyyy");
    } else {
        date = QDate::currentDate().toString("dd-MM-yyyy");
    }

    QTimeEdit* heureWidget = findTimeEditPaiement("heure");
    if (!heureWidget) {
        heureWidget = findTimeEditPaiement("timeEdit_heure_paiement");
    }
    
    QString heure;
    if (heureWidget) {
        heure = heureWidget->time().toString("hh:mm");
    } else {
        heure = QTime::currentTime().toString("hh:mm");
    }

    QLineEdit* montantWidget = findLineEditPaiement("montant");
    if (!montantWidget) {
        montantWidget = findLineEditPaiement("lineEdit_montant_paiement");
    }
    
    if (!montantWidget) {
        QMessageBox::warning(this, "Erreur", "Widget Montant introuvable !");
        return;
    }
    
    QString montantText = montantWidget->text().trimmed();
    ...
    Paiement p(id, type, date, heure, montant);
    if (p.ajouter()) {
...
void application_integree::on_pushButton_modifier_paiement_clicked()
{
    if (selectedIdPaiement <= 0) {
        QMessageBox::warning(this, "Erreur", "Veuillez sélectionner un paiement à modifier !");
        return;
    }

    // Utiliser les fonctions utilitaires
    QComboBox* typeWidget = findComboBoxPaiement("Type");
    if (!typeWidget) typeWidget = findComboBoxPaiement("comboBox_type_paiement");
    ...
    QDateEdit* dateWidget = findDateEditPaiement("Date");
    if (!dateWidget) dateWidget = findDateEditPaiement("dateEdit_date_paiement");
    ...
    QTimeEdit* heureWidget = findTimeEditPaiement("heure");
    if (!heureWidget) heureWidget = findTimeEditPaiement("timeEdit_heure_paiement");
    ...
    QLineEdit* montantWidget = findLineEditPaiement("montant");
    if (!montantWidget) montantWidget = findLineEditPaiement("lineEdit_montant_paiement");
    ...
    Paiement p(selectedIdPaiement, type, date, heure, montant);
    if (p.modifier()) {
        QMessageBox::information(this, "Succès", "Paiement modifié avec succès !");
        afficherPaiements();
        clearFieldsPaiement();
        selectedIdPaiement = 0;
    } else {
        QMessageBox::critical(this, "Erreur", "Échec de la modification du paiement.");
    }
}

void application_integree::on_pushButton_supprimer_paiement_clicked()
{
    if (selectedIdPaiement <= 0) {
        QMessageBox::warning(this, "Erreur", "Veuillez sélectionner un paiement à supprimer !");
        return;
    }
    ...
    if (reply == QMessageBox::Yes) {
        if (Paiement::supprimer(selectedIdPaiement)) {
...
void application_integree::on_tableView_paiements_clicked(const QModelIndex &index)
{
    if (!index.isValid()) return;
    ...
    int row = index.row();
    selectedIdPaiement = model->index(row, 0).data().toInt();

    // Remplir les champs avec les données sélectionnées (utiliser fonctions utilitaires)
    QLineEdit* idWidget = findLineEditPaiement("ID");
    if (!idWidget) idWidget = findLineEditPaiement("lineEdit_id_paiement");
    if (idWidget) {
        idWidget->setText(QString::number(selectedIdPaiement));
    }
    ...
    if (!typeWidget) typeWidget = findComboBoxPaiement("comboBox_type_paiement");
    if (typeWidget) {
        typeWidget->setCurrentText(type);
    }

    QString dateStr = model->index(row, 2).data().toString();
    QDate date = QDate::fromString(dateStr, "dd-MM-yyyy");
    if (date.isValid()) {
        QDateEdit* dateWidget = findDateEditPaiement("Date");
        if (!dateWidget) dateWidget = findDateEditPaiement("dateEdit_date_paiement");
    ...
    QString heureStr = model->index(row, 3).data().toString();
    QTime heure = QTime::fromString(heureStr, "hh:mm");
    if (heure.isValid()) {
        QTimeEdit* heureWidget = findTimeEditPaiement("heure");
        if (!heureWidget) heureWidget = findTimeEditPaiement("timeEdit_heure_paiement");
        if (heureWidget) {
            heureWidget->setTime(heure);
        }
    }
    ...
    if (!montantWidget) montantWidget = findLineEditPaiement("lineEdit_montant_paiement");
    if (montantWidget) {
        montantWidget->setText(montant);
    }
}

void application_integree::clearFieldsPaiement()
{
    // Utiliser les fonctions utilitaires pour trouver et nettoyer les widgets
    QLineEdit* idWidget = findLineEditPaiement("ID");
    if (!idWidget) idWidget = findLineEditPaiement("lineEdit_id_paiement");
    if (idWidget) idWidget->clear();

    QComboBox* typeWidget = findComboBoxPaiement("Type");
    if (!typeWidget) typeWidget = findComboBoxPaiement("comboBox_type_paiement");
    if (typeWidget) typeWidget->setCurrentIndex(0);
    ...
    QLineEdit* montantWidget = findLineEditPaiement("montant");
    if (!montantWidget) montantWidget = findLineEditPaiement("lineEdit_montant_paiement");
    if (montantWidget) montantWidget->clear();

    selectedIdPaiement = 0;
}

void application_integree::on_pushButton_retour_paiements_clicked()
{
    ui->stackedWidget->setCurrentIndex(1); // Retour au menu principal
    clearFieldsPaiement();
    QTableView* tableView = findTableViewPaiement();
    if (tableView) {
        tableView->clearSelection();
...
void application_integree::on_ajouter_paiement_clicked()
{
    on_pushButton_ajouter_paiement_clicked();
}

void application_integree::on_modifier_paiement_clicked()
{
    on_pushButton_modifier_paiement_clicked();
}

void application_integree::on_supprimer_paiement_clicked()
{
    on_pushButton_supprimer_paiement_clicked();
}

void application_integree::on_Retour_paiement_clicked()
{
    on_pushButton_retour_paiements_clicked();
}

void application_integree::on_tabView_clicked(const QModelIndex &index)
{
    on_tableView_paiements_clicked(index);
}
