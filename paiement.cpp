...
bool Paiement::ajouter()
{
    ...
    if (id_apprenant > 0) {
        ...
        query.bindValue(":id_apprenant", QVariant(QVariant::Int)); // NULL
    }
...
QSqlQueryModel* Paiement::afficher() const
{
    ...
    model->setQuery(QSqlQuery("SELECT ID_PAIEMENT, TYPE_PAIEMENT, "
                              "TO_CHAR(DATE_PAIEMENT,'DD-MM-YYYY') AS DATE_PAIEMENT, "
                              "HEURE_PAIEMENT, MONTANT, ID_APPRENANT "
                              "FROM PAIEMENTS", QSqlDatabase::database("oracle_connection")));
...
QSqlQueryModel* Paiement::trierParDate(bool croissant) const
{
    QSqlQueryModel* model = new QSqlQueryModel();
    ...
    model->setQuery(
        ...
        "TO_CHAR(DATE_PAIEMENT,'DD-MM-YYYY'), HEURE_PAIEMENT, MONTANT, ID_APPRENANT "
        "FROM PAIEMENTS ORDER BY DATE_PAIEMENT " + ordre,
...
QSqlQueryModel* Paiement::rechercherParApprenant(int id_apprenant) const
{
    ...
    query.prepare("SELECT ID_PAIEMENT, TYPE_PAIEMENT, "
                  "TO_CHAR(DATE_PAIEMENT,'DD-MM-YYYY'), HEURE_PAIEMENT, "
                  "MONTANT, ID_APPRENANT "
                  "FROM PAIEMENTS WHERE ID_APPRENANT = :id");
